...
Commands:
  setup          First-run wizard: pick provider + key, niche, platforms, CTA
...
def main(argv: list[str]):
    ...
    if cmd == "setup":
        from factory import setup_wizard
        setup_wizard.run()
        return
    # first-run nudge: no .env yet and they're trying to do real work
    from factory.config import ROOT
    if not (ROOT / ".env").exists():
        console.print("[yellow]No .env found — run [cyan]python run.py setup[/] first "
                      "to add your API key and settings.[/]\n")
    if cmd == "find":
        if not urls:
